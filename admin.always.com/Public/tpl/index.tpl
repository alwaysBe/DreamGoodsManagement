 ...
 <div class="row">
    <div class="col-md-12">
        <h1 class="page-header">
            Tables Page <small class='title'><?php echo $title?>列表</small>
        </h1>
    </div>
</div>
                        ...
                        <div class="panel-body">
                            <div class="table-responsive">
                                ...
                                <table class="table table-striped table-bordered table-hover dataTable no-footer" id="dataTables-example" aria-describedby="dataTables-example_info">
                                    <thead>
                                        <tr role="row">
                                          <th class="sorting_asc" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" aria-label="Rendering engine: activate to sort column ascending" style="width: 165px;" aria-sort="ascending"><input type="checkbox" name="ids" class='ids'>#</th>
                                          <?php foreach($fields as $item){
                                            if($item['name']===false){
                                                continue;
                                            }
                                            echo "<th class='sorting' tabindex='0' aria-controls='dataTables-example' rowspan='1' colspan='1' aria-label='Browser: activate to sort column ascending' style='width: 244px;'>".$item['name']."</th>";
                                            }
                                            ?>
                                          <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" aria-label="CSS grade: activate to sort column ascending" style="width: 97px;" colspan="2">操作</th></tr>
                                    </thead>
                                    <tbody>
                                    <volist name='rows' id='row'>
                                      <tr class="gradeA odd">
                                        <td class="sorting_1"><input type="checkbox" name="id[{$row.id}]" class='id'>{$row.id}</td>
                                        ...
                                        <?php foreach($fields as $row){
                                            ...
                                            if($row['field']=='status'){
                                                echo "<td class=\"sorting_1\"><a href=\"{:U('changeStatus',array('id'=>\$row['id'],'status'=>1-\$row['status']))}\" class=\"get_json\"><img src=\"__IMG__/{\$row.status}.png\">
                                              </a></td>";
                                              continue;
                                            }
                                            echo "<td class=\"sorting_1\">{\$row.{$row['field']}}</td>";
                                        }?>
                                        <td>
                                            <a href="{:U('edit',array('id'=>$row['id']))}">编辑</a>
                                            <a href="{:U('changeStatus',array('id'=>$row['id']))}" role="button" data-toggle="modal" class='get_json'>删除</a>
                                        </td>
                                      </tr>
                                    </volist>
